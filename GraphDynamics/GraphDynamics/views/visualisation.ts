export class Home {
    public diagramGroup: d3.Selection<any>;
    public diagram: any[] = [];
    public loaded_graph;
    public valueX = 0;
    public valueY = 3;
    ...
    attached() {
        ...
        for (var i = 0; i < data.flows.length; i++) {
            ...
            flow.cont.forEach(function (x) {
                ...
                nodes.push(sourceId);
                nodes.push(destId);
                if (edges.filter((x) => { return (x.source == sourceId) && (x.target == destId) && (x.connection_type == "cont"); }).length == 0) {
            ...
            flow.merge.forEach(function (x) {
                ...
                x.prev.forEach(function (y) {
                    y.forEach(function (z) {
                        ...
                        nodes.push(sourceId);
                        if (edges.filter((x) => { return (x.source == sourceId) && (x.target == destId) && (x.connection_type == "merge"); }).length == 0) {
            ...
            flow.split.forEach(function (x) {
                ...
                x.next.forEach(function (y) {
                    y.forEach(function (z) {
                        ...
                        nodes.push(destId);
                        if (edges.filter((x) => { return (x.source == sourceId) && (x.target == destId) && (x.connection_type == "split"); }).length == 0) {
    ...
    createNewDiagram(graph) {
        ...
        var node = <any>this.diagramGroup.append("g").selectAll(".node").data(graph.nodes)
